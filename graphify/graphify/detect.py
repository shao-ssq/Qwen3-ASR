...
核心职责: 扫描目录，识别支持的文件并按类型分类，过滤噪音/敏感文件，统计语料库规模。

公共方法:
    detect(root)              - 主入口，递归扫描目录 → {files, total_files, total_words, warning, ...}
    classify_file(path)       - 按扩展名和内容判断类型 (CODE/DOCUMENT/PAPER/IMAGE/VIDEO)
    detect_incremental(root)  - 增量检测，对比 mtime 清单返回新增/修改文件 (--update 模式用)
    count_words(path)         - 统计词数，支持 PDF/DOCX/XLSX 转换后计数
    convert_office_file()     - 将 .docx/.xlsx 转为 Markdown 侧车文件供子代理处理
    load_manifest()           - 加载上次运行的文件修改时间清单
    save_manifest(files)      - 保存当前文件 mtime 供下次增量对比

内部辅助方法:
    _is_sensitive(path)       - 检测密钥文件 (.env/.pem/id_rsa 等)，命中则跳过
    _looks_like_paper(path)   - 启发式检测学术文本 (arXiv/DOI/citation 等 3+ 信号 → PAPER)
    _is_noise_dir(part)       - 判断是否为 venv/node_modules/__pycache__ 等噪音目录
    _load_graphifyignore()    - 递归查找 .graphifyignore 直到 .git 边界
    _is_ignored(path)         - 判断路径是否匹配 .graphifyignore 模式

关键设计:
    - 敏感文件自动跳过 (.env/密钥/凭据)
    - Office 文件 (.docx/.xlsx) 自动转 Markdown
    - .graphifyignore 类似 .gitignore 支持递归查找
    - 增量更新通过 mtime 对比只处理变更文件
    - 语料库健康检查: <50K 词提示"可能不需要图谱"，>500K 提示"提取成本高"
"""
