"""图分析: 上帝节点、意外连接、建议问题、图谱差异对比。

核心职责: 分析已聚类的图谱，识别核心实体（上帝节点）、跨社区意外连接、
         生成建议问题，支持图谱版本差异对比。

公共方法:
    god_nodes(G, top_n)          - 返回连接度最高的 top_n 真实实体
    surprising_connections(G, communities, top_n) - 查找跨文件/跨社区的意外连接
    suggest_questions(G, communities, labels, top_n) - 基于图结构生成建议问题
    graph_diff(G_old, G_new)     - 对比两个图谱快照，返回新增/删除的节点和边

内部辅助方法:
    _node_community_map()        - 反转社区字典: node_id → community_id
    _is_file_node()              - 判断是否为文件级枢纽节点或 AST 方法存根 (排除在分析外)
    _is_concept_node()           - 判断是否为手动注入的语义概念节点 (无 source_file)
    _file_category()             - 按扩展名判断文件类型 (code/paper/image/doc)
    _top_level_dir()             - 提取路径第一级组件，用于检测跨仓库边
    _surprise_score()            - 计算边的意外程度分数 (置信度/跨类型/跨社区/外围→枢纽)
    _cross_file_surprises()      - 多文件语料库: 按综合意外分数排序的跨文件边
    _cross_community_surprises() - 单文件语料库: 跨社区边界边或高介数中心性边

关键设计:
    - 上帝节点排除文件枢纽 (_is_file_node) 和概念节点 (_is_concept_node)
    - 意外连接分两种策略:
        多文件: 跨文件边按 _surprise_score 排序 (AMBIGUOUS > INFERRED > EXTRACTED)
        单文件: 跨社区边或高介数中心性边，按置信度排序去重
    - 意外分数包含: 置信度权重 + 跨文件类型奖励 + 跨仓库奖励 + 跨社区奖励 + 语义相似奖励 + 外围→枢纽奖励
    - 建议问题 5 种类型: ambiguous_edge / bridge_node / verify_inferred / isolated_nodes / low_cohesion
    - 图谱差异支持有向/无向图的边对比，生成人类可读的变更摘要
"""
