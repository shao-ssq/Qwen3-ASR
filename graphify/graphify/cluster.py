"""Leiden/Louvain 社区检测 + 内聚度评分。

核心职责: 在 NetworkX 图上检测社区（连通子图），分割过大社区，计算内聚度分数。

公共方法:
    cluster(G)              - 主入口，社区检测 + 分割 → {community_id: [node_ids]}
    cohesion_score(G, nodes) - 单个社区内聚度 (实际边数/最大可能边数) → 0.0~1.0
    score_all(G, communities) - 批量计算所有社区内聚度 → {id: score}

算法流程:
    1) 有向图 → 无向图转换 (Leiden/Louvain 要求无向输入)
    2) 孤立节点提取 → 各自成为单节点社区
    3) 连通子图跑 _partition() → Leiden 优先，无 graspologic 则 fallback Louvain
    4) 过大社区分割 (>25% 图节点且 ≥10) → 子图再跑一轮 Leiden
    5) 按大小降序重新编号社区 ID (0 = 最大)

关键设计:
    - graspologic 输出被抑制 (stdout/stderr → devnull)，防止 PowerShell 5.1 缓冲区被 ANSI 码破坏
    - Louvain 兼容: max_level 参数仅在 NetworkX 新版本可用，防止大图卡死
    - 无连通边时直接返回每个节点独立社区
"""
