"""确定性 AST 结构提取 — 使用 tree-sitter 解析 20+ 种编程语言。

核心职责: 从源代码文件中提取类、函数、导入、调用关系等结构信息，输出 nodes+edges 字典。

公共方法:
    extract(paths)            - 主入口，批量提取文件列表 → {nodes, edges, input_tokens, output_tokens}
    collect_files(target)     - 递归收集目标路径下所有支持的代码文件
    extract_python/js/go/...  - 各语言专用提取器（通过 _DISPATCH 字典按扩展名分发）

核心架构:
    LanguageConfig dataclass  - 每种语言的 tree-sitter 配置（类/函数/导入 AST 类型、名称字段等）
    _extract_generic()        - 通用 AST 提取器，由 LanguageConfig 驱动，适用于大多数语言
    _walk()                   - 递归遍历 AST 树，提取类、函数、导入、调用、静态属性等
    walk_calls()              - 第二轮遍历函数体，提取调用关系 → INFERRED calls 边

语言提取器 (20+):
    Python   - extract_python()   额外提取 rationale 注释块 (# RATIONALE: ...)
    JS/TS    - extract_js()       支持 import/require/箭头函数/类
    Go       - extract_go()       支持 struct/interface/method/import
    Rust     - extract_rust()     支持 struct/enum/trait/impl/fn/use
    Java     - extract_java()     支持 class/interface/enum/method/import
    C/C++    - extract_c/cpp()    支持 struct/enum/typedef/function/include (declarator 解包)
    Ruby     - extract_ruby()     支持 class/module/def/include
    C#       - extract_csharp()   支持 class/namespace/method/using/属性
    Kotlin   - extract_kotlin()   支持 class/object/fun/interface/import
    Scala    - extract_scala()    支持 class/trait/def/import
    PHP      - extract_php()      支持 class/function/use/trait
    Blade    - extract_blade()    Laravel 模板 (extends/component/bind)
    Dart     - extract_dart()     支持 class/void/final/import/export
    Lua      - extract_lua()      支持 function/local/require
    Swift    - extract_swift()    支持 class/struct/enum/func/import/协议遵循
    Julia    - extract_julia()    支持 struct/function/abstract module
    Zig      - extract_zig()      支持 struct/pub/fn/const/import
    PowerShell- extract_powershell() 支持 class/function/param/using
    ObjC     - extract_objc()     支持 @interface/@implementation/@protocol
    Elixir   - extract_elixir()   支持 defmodule/def/defp/defmacro/import

内部辅助方法:
    _make_id()                - 生成稳定的小写下划线分隔节点 ID
    _read_text()              - 从 AST 节点提取源码文本
    _resolve_name()           - 按 LanguageConfig 配置解析节点名称
    _find_body()              - 查找函数/类体节点
    _import_python/js/...()   - 各语言专用导入处理器
    _resolve_cross_file_imports() - Python 跨文件导入解析 (文件级 → 类级 INFERRED 边)
    _extract_python_rationale() - 提取 Python 中的 rationale 注释块

关键设计:
    - 两阶段提取: 1) 单文件 AST 解析  2) 跨文件导入解析 (Python only)
    - SHA256 文件哈希缓存: 未变更文件跳过重复提取
    - 置信度标签: EXTRACTED (明确) / INFERRED (推导调用关系)
    - 文件内去重: seen_ids 集合确保同一节点 ID 每文件只出现一次
    - 进度报告: 每 100 个文件打印一次进度
"""
