"""多格式图导出: HTML/JSON/SVG/GraphML/Obsidian 仓库/Neo4j Cypher。

核心职责: 将 NetworkX 图 + 社区信息导出为各种可视化/数据格式。

公共方法:
    to_json(G, communities, output_path)   - 导出为 GraphRAG-ready JSON (nodes/edges/communities)
    to_html(G, communities, output_path)   - 交互式 HTML 可视化 (vis.js 力导向图 + 侧边栏 + 图例)
    to_svg(G, communities, output_path)    - SVG 静态图 (matplotlib 绘制，可嵌入 Notion/GitHub)
    to_graphml(G, communities, output_path)- GraphML 格式 (Gephi/yEd 兼容)
    to_obsidian(G, communities, out_dir)   - Obsidian 知识库 (每节点一个 .md 文件 + 社区概览)
    to_canvas(G, communities, output_path) - Obsidian Canvas 文件 (结构化社区布局)
    to_cypher(G, output_path)              - Neo4j Cypher 脚本 (可导入 cypher-shell)

内部辅助方法:
    _html_styles()          - 暗色主题 CSS 样式 (搜索框 + 信息面板 + 图例 + 统计)
    _html_script()          - vis.js 力导向图交互脚本 (点击节点、搜索、图例过滤、超边绘制)
    _hyperedge_script()     - 超边 Canvas 绘制 (凸多边形半透明填充)
    _strip_diacritics()     - 去除变音符号 (用于 safe_name 清理)
    _cypher_escape()        - Neo4j 标识符转义 (反引号包裹)
    _safe_name()            - 清理文件名/URL 中的非法字符
    _build_node_info()      - 构建节点信息 HTML (属性 + 邻居列表)
    _write_obsidian_note()  - 写入单个 Obsidian .md 笔记 (含 frontmatter + 连接)

关键设计:
    - HTML 可视化限制 5000 节点上限 (超过则提示用 Obsidian)
    - 社区颜色: 10 色调色板循环分配
    - Obsidian 导出: 每节点一个 .md + _COMMUNITY_*.md 概览 + graph.canvas 布局
    - Canvas 文件: 社区作为分组框，节点按社区排列
    - JSON 包含 pruned_dangling_count 统计被修剪的悬空边数
    - Neo4j 使用 MERGE 语句，可安全重复执行不产生重复节点
"""
